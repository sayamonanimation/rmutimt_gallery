...
if ($projectId <= 0) {
    ...
    header('Location: ' . manage_list_state_url('./manage_projects.php', $_GET));
    exit;
}
...
$pageSubtitle = 'Admin | Dashboard | Edit Project';
$pageSubtitleDataTh = 'ผู้ดูแล | แดชบอร์ด | แก้ไขโปรเจกต์';
$pageSubtitleDataEn = 'Admin | Dashboard | Edit Project';
$manageProjectsReturnUrl = manage_list_state_url('./manage_projects.php', $_GET);
$manageProjectsSuccessUrl = manage_list_state_url('./manage_projects.php', $_GET, 'success');
?>
<!doctype html>
<html lang="th">
<head>
...
<body class="h-screen overflow-hidden dark:bg-[#121212] dark:text-gray-100">
    ...
    <div class="flex-1 flex flex-col h-full min-h-0 overflow-hidden">
      <main class="flex-1 overflow-y-auto p-6 md:p-8">
        <div class="mx-auto max-w-5xl">
          ...
          <form method="POST" action="./update_process.php" enctype="multipart/form-data" class="mt-6 space-y-6">
            <?= csrf_input() ?>
            <input type="hidden" name="project_id" value="<?= (int) $projectId ?>">
            <input type="hidden" name="return_page" value="<?= max(1, (int) ($_GET['page'] ?? 1)) ?>">
            <input type="hidden" name="return_search" value="<?= htmlspecialchars(trim((string) ($_GET['search'] ?? '')), ENT_QUOTES, 'UTF-8') ?>">
            ...
            <section class="rounded-2xl border border-[var(--border)] bg-white dark:bg-[#1e1e1e] dark:border-[#333333] p-5 md:p-6">
              ...
              <div class="mt-4">
                ...
                <textarea id="edit-introduction" class="form-textarea dark:bg-[#1e1e1e] dark:border-gray-700 dark:text-white dark:placeholder-gray-500 dark:focus:border-[var(--mt-red)] dark:focus:ring-[var(--mt-red)]/20" name="introduction" maxlength="3500" required><?= htmlspecialchars($introduction, ENT_QUOTES, 'UTF-8') ?></textarea>
                <div class="mt-1 text-right text-[10px] text-gray-400"><span id="edit-intro-char-count">0</span>/3500</div>
              </div>
            </section>
            ...
            <div class="w-full pb-4">
              ...
              <a href="<?= htmlspecialchars($manageProjectsReturnUrl, ENT_QUOTES, 'UTF-8') ?>" class="inline-flex h-11 items-center gap-2 rounded-xl border border-[#DCE0E5] bg-white px-6 text-sm font-semibold text-[#555F6D] transition-all duration-200 hover:-translate-y-[1px] hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:border-[#333333] dark:bg-[#1e1e1e] dark:text-gray-300 dark:hover:bg-gray-800">Back to Manage Projects</a>
              </div>
            </div>
          </form>
        </div>
  ...
  <script>
    (function () {
      const uploadForm = document.querySelector('form[action="./update_process.php"]');
      ...
      if (uploadForm instanceof HTMLFormElement && submitBtn instanceof HTMLButtonElement) {
        const successRedirect = <?= json_encode($manageProjectsSuccessUrl) ?>;
        ...
        uploadForm.addEventListener('submit', function (e) {
          ...
          const formSections = uploadForm.querySelectorAll('section');
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const introTextarea = document.getElementById('edit-introduction');
    const introCounter = document.getElementById('edit-intro-char-count');
    if (introTextarea && introCounter) {
      introCounter.textContent = introTextarea.value.length;
