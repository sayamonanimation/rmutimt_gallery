...
{
    if (!headers_sent()) {
        header('Location: ./my_projects.php?status=success');
    } else {
        echo '<script>window.location.href="./my_projects.php?status=success";</script>';
        echo '<noscript><meta http-equiv="refresh" content="0;url=./my_projects.php?status=success"></noscript>';
    }
    exit;
}
...
function projectsTableHasDriveFolderId(PDO $pdo): bool
{
    ...
    try {
        $stmt = $pdo->query("SELECT 1 FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'projects' AND column_name = 'drive_folder_id' LIMIT 1");
...
// Release session lock so user can browse other tabs/pages during heavy file upload
if (session_status() === PHP_SESSION_ACTIVE) {
    session_write_close();
}
...
try {
    ...
    $insertStmt = $pdo->prepare($insertSql . ' RETURNING id');
    $insertStmt->execute($params);

    $projectId = (int) $insertStmt->fetchColumn();
