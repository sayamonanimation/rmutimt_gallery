...
$pageSubtitle = 'Student | Dashboard | Upload New Project';
$pageSubtitleDataTh = 'นักศึกษา | แดชบอร์ด | อัปโหลดโปรเจกต์ใหม่';
$pageSubtitleDataEn = 'Student | Dashboard | Upload New Project';
$advisors = [];
$categories = [];
$academicYears = [];
    ...
    <div class="flex-1 flex flex-col h-full min-h-0 overflow-hidden">
      <main class="flex-1 overflow-y-auto p-6 md:p-8">
        <div class="mx-auto max-w-5xl">
          ...
          <form id="upload-project-form" method="POST" action="./upload_process_student.php" enctype="multipart/form-data" class="mt-6 space-y-6">
      ...
      if (uploadForm instanceof HTMLFormElement && submitBtn instanceof HTMLButtonElement) {
        ...
        uploadForm.addEventListener('submit', function (e) {
          ...
          if (validationErrors.length > 0 || !uploadForm.checkValidity()) {
            ...
            document.body.dataset.lastValidationError = validationErrors.join(' | ') || 'checkValidity_failed';
          ...
          const thumbIn = document.getElementById('thumbnail_file');
          if (thumbIn instanceof HTMLInputElement && thumbIn.files && thumbIn.files.length > 0) {
            formData.set('thumbnail_file', thumbIn.files[0]);
          }
          const pdfIn = document.getElementById('thesis_pdf_file');
          if (pdfIn instanceof HTMLInputElement && pdfIn.files && pdfIn.files.length > 0) {
            formData.set('thesis_pdf_file', pdfIn.files[0]);
          }
          ...
          xhr.onload = function () {
            if (xhr.status >= 200 && xhr.status < 400) {
              ...
              window.location.href = xhr.responseURL || './my_projects.php?status=success';
            } else {
